...
void Character::updateSprite(void) {
...
void Character::drawMessage(sf::RenderWindow &window)
{
	...
	message.setPosition(this->getPosition().x + 100, this->getPosition().y - 200);
...
void Character::checkCollisionGoal(map<string, pair<string, GameObject*>> gameObjects)
{
	...
	while (itr != gameObjects.end())
	{
		if ("goal" == itr->second.first)
		{
			...
			if (getBoundingRect().intersects(goal->getBoundingRect())) 
			{
